import {
  ...
  Alert,
  ScrollView,
  TouchableOpacity, // Ensure TouchableOpacity is imported from react-native
  Platform,
  ActionSheetIOS, // For Platform specific checks if needed for DateTimePicker
} from "react-native";
import React, { useState } from "react";
// import { TouchableOpacity } from "react-native"; // Already imported above
...
const RegisterSection = () => {
  ...
  const [gender, setGender] = useState(""); // Store as true (male), false (female), or "" (not selected)
  const [dob, setDob] = useState(null); // Store Date object, null initially
  ...
  // Remove or comment out the old date picker functions:
  ...
  const selectGender = () => {
    ActionSheetIOS.showActionSheetWithOptions(
      ...
      (buttonIndex) => {
        if (buttonIndex === 1) {
          setGender(true); // Set to male
          setErrors((prev) => ({ ...prev, gender: null }));
        } else if (buttonIndex === 2) {
          setGender(false); // Set to female
          setErrors((prev) => ({ ...prev, gender: null }));
        }
      }
    );
  ...
  const validate = () => {
    ...
    if (!phoneNumber.trim()) {
      ...
      // Basic 10 digit, starts with 0
      newErrors.phoneNumber = "Số điện thoại không hợp lệ.";
      isValid = false; // Ensure isValid is set to false for this condition
    }
    // Corrected gender and DOB validation to be independent checks
    if (gender === "") {
      // Updated validation for gender
    ...
    // The problematic 'isValid = false;' and mismatched '}' that were here have been removed.
  ...
  const handleRegister = async () => {
    ...
    const apiData = {
      ...
      isMale: gender, // Use gender directly as it's true/false
    };

    console.log("Submitting API Data:", apiData);

    try {
      const response = await api.post("identity-api/register", apiData);

      console.log("API Response:", response.data);

      // Assuming successful registration, navigate to OTP screen
      Alert.alert(
        "Đăng ký thành công",
        "Vui lòng kiểm tra email để xác thực tài khoản." // Or a message from API
      );
      navigation.navigate("otpscreen", { email: apiData.email });
    } catch (error) {
      console.error(
  ...
  return (
    ...
    >
      <DateTimePickerModal
        ...
        date={dob || new Date(1900, 0, 1)} // Default to a very old date
      ...
      <View style={styles.idAndGenderContainer}>
        ...
        <View style={styles.genderInputGroup}>
          ...
          >
            {Platform.OS === "ios" ? (
              <TouchableOpacity onPress={selectGender}>
                <Text
                  style={{
                    padding: 12,
              ...
              <Picker
                ...
                mode="dropdown" // This ensures the picker does not always stay open on Android
              >
                <Picker.Item
